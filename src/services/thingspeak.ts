...
class ThingSpeakService {
  ...
  private parseData(feed: any): ThingSpeakData {
    return {
      ...
      fallDetection: this.parseNumber(feed.field6, 0),
      bloodPressure: {
        systolic: this.parseNumber(feed.field4, 120),
        diastolic: this.parseNumber(feed.field8, 80)
      }
    };
  }
  ...
  private handleError(error: AxiosError): Error {
    ...
    if (error.response) {
      switch (error.response.status) {
        case 400:
          return new Error('Invalid request - please check your channel ID and API key');
        case 401:
          return new Error('Unauthorized - please verify your API key');
        case 404:
          return new Error('Channel not found - please verify your channel ID');
      ...
      return new Error('Unable to reach ThingSpeak servers - check your internet connection');
    }
